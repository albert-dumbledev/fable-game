...
**Juggernaut** (`boss_enemy.gd` extends `BossBase`; `juggernaut.tres`): 750 HP base (× wave HP mult), a warhammer with two distance-picked attacks plus the unchanged charge. `attack_range` is a dummy-large **26** so the base state machine always "attacks" and the script picks the mode by distance:

...
> **Status: ✅ Implemented (M1–M5 shipped; pending playtest tuning + `ENEMIES.md`/`PLAN.md` refresh).** Reworks the Juggernaut from "big chaser with a charge" into a two-mode warhammer boss (boulder throw at range, telegraphed hammer slam up close), and replaces the minute-5 double-Juggernaut wave with a new **Caster boss** that kites, retaliates with a zero-damage repulse, and carries the staff drop. Collecting the staff **ends the run (victory)** for now.
...
4. **M4 — Eruption + rotation polish:** the third spell, cast priorities, orb-flare/SFX pass (new cues: repulse whoomp, boulder thud, eruption crack — low fundamentals, differentiate by timbre).
   - **M4a — Arcane Eruption:** ✅ **Done.** Three chained rifts (`_cast_eruption` schedules `_spawn_rift` at 0/0.55/1.1 s via `create_timer`; each snapshots the player's *current* position, telegraphs 0.8 s in violet, then `_erupt` hits through the hurtbox for 1.2× dmg + an up-and-out `apply_shove` pop, shoving minions). Sequence outlives the cast's RECOVER; guarded against death/untree. `_choose_spell` priority now Eruption > Fireball > Bolt. **Runtime-validated:** 3 rifts staggered 0.55 s, each erupted ~0.8 s later for exactly 24 dmg (72 total on a parked dummy). Placeholder `explosion` SFX until M4b.
   - **M4b — SFX pass:** ✅ **Done.** Three new procedural cues in `SfxFactory` (`boulder_impact` — rocky boom+debris+crack; `repulse` — low outward whoomp with a rising-cutoff swell; `eruption` — sharp crack over a sub thump + rising geyser tone), all low-fundamental / timbre-differentiated per the audio direction. Wired out the placeholders: boulder → `boulder_impact`, repulse → `repulse`, eruption → `eruption`. The slam keeps `hammer_slam` (it *is* a hammer slam — no new cue called for). Verified all three synthesize + play headless.
5. **M5 — Schedule + victory:** ✅ **Done.** 300 s event swapped from two Juggernauts to one Caster (`data/waves/default.tres`, announcement "THE HIEROPHANT APPROACHES"); staff drop moved off `juggernaut.tres` (→ warhammer only) onto `caster.tres`. Staff claim now ends the run in victory: `RunDirector._on_unlock_claimed` leaves spawning paused and defers to `finish_victory` (called by the ClaimScreen's Continue, so the overlay isn't yanked by an immediate scene change); `finish_victory` banks progress and calls `end_run({victory: true})`. `ClaimScreen` shows run-complete framing ("THE STAFF IS YOURS — RUN COMPLETE" / "Finish") for the staff; `DeathScreen` reads the `victory` flag → "VICTORY" title. Veteran saves that already own the staff get no drop and the run continues endless (no migration needed). **Runtime-validated end-to-end on the real Arena:** claim → paused ClaimScreen with the right framing → Continue → `end_run(victory:true)` → DeathScreen "VICTORY". `docs/ENEMIES.md` (boss section rewritten for both bosses + shared tech + victory flow) and `PLAN.md` (new Phase 6 row, code map, watch notes) refreshed.

extends Node
## Headless smoke harness for the Depths feature M1 (docs/DEPTHS.md). Exercises
## the MetaProgression selection/validation math, then boots the Arena twice —
## once at Depth I (asserting the spawner folds in the Depth mults and a faked
## finale kill records the clear) and once at Surface (asserting today's numbers
## are untouched). Run with:
